...
namespace SmartMIS.UserControl
{
    public partial class CuringProductionReport : System.Web.UI.UserControl
    {
                ...
                if (tempString.Length > 1)
                {
                    ...
                    else if (rType == "1")
                    {
                        ...
                        if (rChoice == "0")
                        {
                            ...
                            rToDate = formatToDate(myWebService.formatDate(tempString[3]));
        ...
        protected void showDailyReport(string query)
        {
            try
            {
                ...
                else
                {
                    ...
                    fillGridView("Select DISTINCT wcID, wcName from vCuringProduction1 WHERE " + query + remainingQuery + " ORDER BY wcName ASC");
                }
            }
            catch (Exception exp)
            {
        ...
        private void showMonthlyReport(string query)
        {
            try
            {
                fillGridView("Select DISTINCT wcID, wcName from vCuringProduction1 WHERE " + query + " ORDER BY wcName ASC");
        ...
        private void fillChildGridView(GridView childGridView, String[] args, String toDate, String fromDate)
        {
            ...
            try
            {
                ...
                query = "select recipecode,description,quantity,shift from vCuringProduction1 where wcname='" + args[0] + "' AND  dtandtime>='" +rToDate+ "' AND dtandtime<='" +rFromDate + "'" + remainingQuery;
                ...
                for (int i = 0; i < uniqrecipedt.Rows.Count; i++)
                {
                    ...
                    dr[2] = dt.Compute("COUNT(quantity)", "recipecode='" + uniqrecipedt.Rows[i][0].ToString() + "' AND  shift='A'");
                    dr[3] = dt.Compute("COUNT(quantity)", "recipecode='" + uniqrecipedt.Rows[i][0].ToString() + "' AND  shift='B'");
                    dr[4] = dt.Compute("COUNT(quantity)", "recipecode='" + uniqrecipedt.Rows[i][0].ToString() + "' AND  shift='C'");
                    dr[5] = dt.Compute("COUNT(quantity)", "recipecode='" + uniqrecipedt.Rows[i][0].ToString() + "'");
