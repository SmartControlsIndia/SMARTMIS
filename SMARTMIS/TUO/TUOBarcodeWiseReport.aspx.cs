            ...
            try
            {
                ...
                if (DateFromToReport.Checked)
                {
                    ...
                    try
                    {
                        ...
                        myConnection.comm.CommandText = "Select MachineName,dtandTime as TestTime, tireType AS TyreType ,BARCODE ,static, staticAngle,staticGrade,couple,coupleAngle,coupleGrade,upper,upperAngle,uppergrade,lower,lowerAngle,lowergrade, GradeDef FROM vproductionDataPCRDB where (dtandTime>'" + Convert.ToDateTime(fromdate).ToString("yyyy-MM-dd HH:mm:ss") + "' AND dtandTime<'" + Convert.ToDateTime(todate).AddDays(1).ToString("yyyy-MM-dd HH:mm:ss") + "')";
                        myWebService.writeLogs(myConnection.comm.CommandText, System.Reflection.MethodBase.GetCurrentMethod().Name, Path.GetFileName(Request.Url.AbsolutePath));
