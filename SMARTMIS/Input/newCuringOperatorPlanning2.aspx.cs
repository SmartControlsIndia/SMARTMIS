...
namespace SmartMIS.Input
{
    public partial class newCuringOperatorPlanning2 : System.Web.UI.Page
    {
        ...
        private void fillmanningdropdownlistTBR()
        {
            manningDropDownList.DataSource = null;
            manningDropDownList.DataSource = FillDropDownList("vmanning", "firstname, lastname", "where areaname='" + areaname + "' or areaname='VITBR'  ORDER BY firstname ASC");
        ...
        private bool iswcgroupExist()
       {
        ...
        protected void GridView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (PCRCuringRedioButton.Checked)
                ...
                //areaname ="VITBR" + areaname="TBRCuring";
              areaname="TBRCuring";
        ...
        protected bool saveinfo(string sapCode, string wcGID, string shift)
        {
            try
            {
                ...
                if (sapCode == "None" || string.IsNullOrEmpty(sapCode))
                {
                    myConnection.open(ConnectionOption.SQL);
                    myConnection.comm = myConnection.conn.CreateCommand();
                ...
                else if (sapCode != "Select" && !string.IsNullOrEmpty(sapCode))
                {
        ...
        {
            string[,] opA = new string[30,20];
            string[,] opB = new string[30,20];
            string[,] opC = new string[30,20];
            ...
            foreach (GridViewRow row in operatorPlanningMainGridView.Rows)
            {
                ...
                if (shiftAoperatorCode != null)
                {
                    for (int j = 0; j <= i; j++)
                    {
                        if (opA[j, 0] == shA && shA != "Select" && shA != "None" && !string.IsNullOrEmpty(shA))
                        {
                ...
                if (shiftBoperatorCode != null)
                {
                    for (int j = 0; j <= i; j++)
                    {
                        if (opB[j, 0] == shB && shB != "Select" && shB != "None" && !string.IsNullOrEmpty(shB))
                        {
                ...
                if (shiftCoperatorCode != null)
                {
                    for (int j = 0; j <= i; j++)
                    {
                        if (opC[j, 0] == shC && shC != "Select" && shC != "None" && !string.IsNullOrEmpty(shC))
                        {
