...
namespace SmartMIS.Report
{
    public partial class TBRVISummaryReport : System.Web.UI.Page
    {
        ...
        private DataSet fillFaltName(string faltType, string faltArea, string recipeCode, string rTodate, string rFromDate, ConnectionOption option)
        {
            ...
            myConnection.comm.CommandText = "select distinct defectName as faultName from  vTBRVisualInspectionReport  where status='" + faltType + "' and CuringRecipeID='" + recipeCode + "' and defectAreaID='" + faltArea + "'  and ((dtandTime>=" + rToDate + " AND wcName IN ('7001','7002','7005','7006','7007')";
        ...
        protected void FaultTypeDropDownList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            rToDate = TotaldtformatDate(tuoReportMasterFromDateTextBox.Text, tuoReportMasterToDateTextBox.Text);
            fillRecipeWiseGridView("select distinct curingRecipeID, curingRecipeName from vTBRVisualInspectionReport where ((dtandTime>=" + rToDate + " AND wcName IN ('7001','7002','7005','7006','7007')");
        ...
        public int TyreQuantity(Object FaultName)
        {
            ...
            myConnection.comm.CommandText = "select distinct count(*) from  vTBRVisualInspectionReport  where defectName='" + FaultName + "' and  CuringRecipeID='" + recipeID + "' and status='" + faultname + "' and defectAreaID='" + faultAreaID + "' and ((dtandTime>=" + rToDate + " AND wcName IN ('7001','7002','7005','7006','7007')";
        ...
        private void fillBarCodeDetailGridView(string recipecode,string status)
        {
            try
            {
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "select TBMwcName, curingWCName, mouldNo AS mouldName, wcname AS visualWCName, description AS size, gtbarCode AS barcode from vTBRVisualInspectionExcelReport where status='" + status + "' AND wcName IN ('7001','7002','7005','7006','7007') AND ((dtandTime>=" + rToDate + recipecode;
                myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                dt.Load(myConnection.reader);

                performanceReportBarcodeDetailGridView.DataSource = dt;
                ...
                if (dt.Rows.Count != 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        try
                            {
                                dr[1] = (from r in curdt.AsEnumerable()
                                         where r.Field<string>("gtbarCode") == dt.Rows[i][2].ToString()
                                                               select r.Field<string>("wcName")).First<string>();
                                dr[2] = (from r in curdt.AsEnumerable()
                                         where r.Field<string>("gtbarCode") == dt.Rows[i][2].ToString()
                                         select r.Field<string>("mouldNo")).First<string>();
                            }
                            catch (Exception e) { }
                        
                        try
                            {
                                dr[0] = (from r in tbmdt.AsEnumerable()
                                         where r.Field<string>("gtbarCode") == dt.Rows[i][2].ToString()
                                         select r.Field<string>("wcName")).First<string>();
                            }
                            catch (Exception e) { }
        ...
        protected void expToExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = "select wcName, description, TBMwcName, convert(char(10), curingdtandTime, 110) AS getdate, convert(char(8), curingdtandTime, 108) AS gettime, curingwcName, mouldNo, ssORnss=(CASE WHEN ssORnss='1' THEN 'SS' WHEN ssORnss='2' THEN 'NSS' WHEN ssORnss='0' THEN '' END), CAST(dtandTime AS DATE) AS getdate, convert(char(8), dtandTime, 108) AS gettime, firstName + ' ' + lastName As builderName, gtbarcode, defectAreaName, defectName, remarks, shift=(CASE WHEN convert(char(8), dtandTime, 108) >= '07:00:00 AM' AND convert(char(8), dtandTime, 108) <= '14:59:59.999' THEN 'A' WHEN convert(char(8), dtandTime, 108) >= '15:00:00.000' AND convert(char(8), dtandTime, 108) <= '22:59:59.999' THEN 'B' WHEN ((convert(char(8), dtandTime, 108) >= '23:00:00.000' AND convert(char(8), dtandTime, 108) <= '23:59:59.999') or (convert(char(8), dtandTime, 108) >= '00:00:01.000' AND convert(char(8), dtandTime, 108) <= '06:59:59.999')) THEN 'C' END) from vTBRVisualInspectionExcelReport where wcName IN ('7001','7002','7005','7006','7007') AND dtandTime>'" + rToDate + "' and dtandTime<='" + rFromDate + "' AND status<>'1'";
                    myConnection.open(ConnectionOption.SQL);
                    myConnection.comm = myConnection.conn.CreateCommand();
                    ...
                    sb.Append("<table border=1><tr style=\"background-color:#FFFF00;\"><th>S. No.</th><th>Inspection PressNo</th><th>Shift</th><th>TyreSize</th><th>Building Machine Name</th><th>Press Date</th><th>Press Time</th><th>Press No.</th><th>Cavity</th><th>Mould No.</th><th>Side</th><th>Building Date</th><th>Building Time</th><th>Builder Name</th><th>Barcode</th><th>Defect Location</th><th>Defect</th><th>Remark</th><th>Responsibility</th></tr>");
                    if (myConnection.reader.HasRows)
                    {
                        int i = 1;
                        while (myConnection.reader.Read())
                        {
                            sb.Append("<tr><td>" + i + "</td><td>" + myConnection.reader[0] + "</td><td>" + myConnection.reader[15] + "</td><td>" + myConnection.reader[1] + "</td><td>" + myConnection.reader[2] + "</td><td>" + myConnection.reader[3] + "</td><td>" + myConnection.reader[4] + "</td><td>" + myConnection.reader[5] + "</td><td></td><td>" + myConnection.reader[6] + "</td><td>" + myConnection.reader[7] + "</td><td>" + myConnection.reader[8] + "</td><td>" + myConnection.reader[9] + "</td><td>" + myConnection.reader[10] + "</td><td>" + myConnection.reader[11] + "</td><td>" + myConnection.reader[12] + "</td><td>" + myConnection.reader[13] + "</td><td>" + myConnection.reader[14] + "</td><td></td></tr>");
                            i++;
                        }
                    }
                    sb.Append("</table>");
                    ...
                    /*SqlCommand cmd = new SqlCommand("SELECT wcName, mouldNo, gtbarCode, convert(char(10), dtandTime, 110) AS getdate, convert(char(8), dtandTime, 108) AS gettime FROM vCuringtbr WHERE gtbarCode IN " + InQuery.ToString(), con);
                      SqlCommand bcmd = new SqlCommand("SELECT wcName, gtbarCode, CAST(dtandTime AS DATE) AS getdate, convert(char(8), dtandTime, 108) AS gettime FROM vTbmTBR WHERE dtandTime>'" + rToDate + "' and dtandTime<='" + rFromDate + "'", bcon);
                        
                        string buildwcName = "<td>" + (from r in builddt.AsEnumerable()
                                                               where r.Field<string>("gtbarCode") == dt.Rows[i][5].ToString()
                                ...
                                sb.Append(buildwcName + "<td>" + "<td>" + newquery[0].getdate.ToString() + "</td><td>" + newquery[0].gettime.ToString() + "</td><td>" + newquery[0].wcName.ToString() + "</td><td></td><td>" + newquery[0].mouldNo.ToString() + "</td>" + dt.Rows[i][9] + "</td><td>" + dt.Rows[i][2] + "</td><td>" + dt.Rows[i][3] + "</td><td>" + dt.Rows[i][4] + "</td><td>" + dt.Rows[i][5] + "</td><td>" + dt.Rows[i][6] + "</td><td>" + dt.Rows[i][7] + "</td><td></td><td>" + dt.Rows[i][8] + "</td><td></td></tr>");
