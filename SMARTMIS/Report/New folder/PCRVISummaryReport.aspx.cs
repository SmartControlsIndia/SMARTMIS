    ...
    public partial class PCRVISummaryReport2 : System.Web.UI.Page
    {
        ...
        private void loadData()
        {
            DataTable dt_vi = new DataTable();
                        ...
                        myConnection.comm.CommandText = "select wcID, curingRecipeID, status, defectStatusID As defectID from VInspectionPCR where ((dtandTime>=" + rToDate + " AND wcID in (select iD from wcmaster where wcGroupID=2 and processID=9)";
                        ...
                        myConnection.comm.CommandText = "select Distinct iD, name from wcMaster where wcGroupID=2 and processID=9";
                        myConnection.reader = myConnection.comm.ExecuteReader();
    ...
    .Where(row => row.Cells[0].Text == "Total");
                ...
                switch(getdisplaytype)
                {
                    case "Numbers":
                        foreach (var item in items)
                        {
                            ...
                            {
                                totalcheckedcount += data.Count();
                                okcount += data.Count(d => d.status == 21);
                                reworkcount += data.Count(d => d.status == 22);
                                ncmrcount += data.Count(d => d.status == 23);

                                dr[0] = data[0].description;
                                dr[1] = data.Count();
                                dr[2] = data.Count(d => d.status == 21);
                                dr[3] = data.Count(d => d.status == 22);
                                dr[4] = data.Count(d => d.status == 23);
                            }
                            gridviewdt.Rows.Add(dr);
                    ...
                    case "Percent":
                        foreach (var item in items)
                        {
                            ...
                            dr[4] = (data.Count(d => d.status == 23) * 100 / Convert.ToInt32(dr[1])) + "%";
                        ...
                        drt[4] = (ncmrcount * 100 / totalcheckedcount) + "%";
                        gridviewdt.Rows.Add(drt);
                        break;
                }
        ...
        private string TotaldtformatDate(String fromDate, String toDate)
        {
            ...
            if (fromDate != null)
            {
                ...
                try
                {
                    ...
                    if (Convert.ToInt32(tday) == 12 && Convert.ToInt32(tmonth) == 21)
                    {
        ...
        private void fillBarCodeDetailGridView(string recipecode,string status)
        {
            try
            {
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "select wcname, description, gtbarCode AS vi_gtbarCode from vVisualInspectionPCR where status='" + status + "' AND   wcID in (select iD from wcmaster where wcGroupID=2 and processID=9) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "'" + recipecode;
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "SELECT wcName, mouldNo, gtbarcode AS cur_gtbarCode FROM vCuringpcr WHERE dtandTime>='" + tempfromdate + "' AND dtandTime<'" + to_date + "'";
                myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "SELECT wcName, gtbarcode AS tbm_gtbarCode FROM vTbmpcr WHERE dtandTime>='" + tempfromdate + "' AND dtandTime<'" + to_date + "'";
                myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
            ...
            try
            {
                ...
                else
                {
                   // rFromDate = formatfromDate(rFromDate.Replace(" 07:00:00", ""));
                    ...
                    string[] head_array = new string[] { "S. No.", "visualWCName", "TyreSize", "Barcode", "Status", "DefectAreaName", "Defectname", "Remark", "shift","InspectorName", "PressNo", "MouldNo", "Cure_Date", "Cure_Time", "tbmWCName", "TBM_Date", "TBM_Time", "BuilderName" };
                    ...
                    from vvisualInspectionPCR where  wcID in (select iD from wcmaster where wcGroupID=2 and processID=9) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "'";
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                    dt.Load(myConnection.reader);
                    myConnection.conn.Close();
                    myConnection.comm.Dispose();
                    ...
                    myConnection.open(ConnectionOption.SQL);
                    myConnection.comm = myConnection.conn.CreateCommand();
                    myConnection.comm.CommandText = "SELECT gtbarCode AS tbm_gtbarcode, wcName AS tbmWCName, CAST(dtandTime AS DATE) AS TBM_Date, convert(char(8), dtandTime, 108) AS TBM_Time, firstName + ' ' + lastName As BuilderName FROM vTbmpcr WHERE dtandTime>='" + tempfromdt + "' AND dtandTime<'" + to_date + "'";
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                    tbmdt.Load(myConnection.reader);
                    myConnection.conn.Close();
                    myConnection.comm.Dispose();
