...
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT ,TBMStatus AS STATUS  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and wcName='" + wcname + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    ...
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT ,TBMStatus AS STATUS  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and recipeCode='" + recipename + "'  and wcName  not in ('TBMPCR12') order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname == "ALL" && recipename != "All" && shift != "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT ,TBMStatus AS STATUS  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and recipeCode='" + recipename + "' and wcName  not in ('TBMPCR12')   and shift='" + shift + "' order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname == "ALL" && recipename == "All" && shift != "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT,TBMStatus AS STATUS   FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and wcName  not in ('TBMPCR12')  and shift='" + shift + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
