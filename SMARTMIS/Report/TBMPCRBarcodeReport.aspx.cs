...
{
    public partial class TBMPCRBarcodeReport : System.Web.UI.Page
    {
        #region classes
...
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and wcName='" + wcname.TrimEnd() + "' and recipeCode='" + recipename + "' and shift='" + shift + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname != "ALL" && recipename == "All" && shift != "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcrwhere dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and wcName='" + wcname.TrimEnd() + "'  and shift='" + shift + "' order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname != "ALL" && recipename == "All" && shift == "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and wcName='" + wcname + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname == "ALL" && recipename != "All" && shift == "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and recipeCode='" + recipename + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else if (wcname == "ALL" && recipename != "All" && shift != "ALL")
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' and recipeCode='" + recipename + "'   and shift='" + shift + "' order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    ...
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "'  and shift='" + shift + "'  order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
                    }
                    else
                    {
                        myConnection.comm.CommandText = @"SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 1)) AS SNo,wcName as MACHINENAME,gtbarcode AS BARCODE,recipeCode AS RECIPE ,SAPMaterialCode AS SAPCODE,
convert(char(10), dtandtime, 105)  as DATE, convert(varchar(8), dtandtime, 108) as TIME,shift AS SHIFT  FROM vtbmpcr where dtandtime>='" + nfromDate + "' and dtandtime<'" + ntoDate + "' order by dtandtime desc";// "select dbo.vCuringtbr.gtbarCode, dbo.vtbmpcr.recipeCode, convert(char(10), vCuringtbr.dtandtime, 105) AS Date,convert(char(10), vCuringtbr.dtandtime, 108) AS Time, dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift FROM  dbo.vCuringtbr INNER JOIN dbo.vtbmpcr ON dbo.vCuringtbr.gtbarCode = dbo.vtbmpcr.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc";
        ...
        private void fillSizedropdownlist()
        {
            ...
            myConnection.open(ConnectionOption.SQL);
            myConnection.comm = myConnection.conn.CreateCommand();
            myConnection.comm.CommandText = "Select DISTINCT id as rID,name from recipemaster where description != '0' and description !='' and processID = 7  order by name desc ";
            myConnection.reader = myConnection.comm.ExecuteReader();
        ...
        protected void expToExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.AddHeader("content-disposition", "attachment;filename=TBMPCRReport.xls");
            Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

            ExcelPackage pck = new ExcelPackage();
            ExcelWorksheet ws = pck.Workbook.Worksheets.Add("TBMPCRReport");
            ws.Cells["A1"].Value = "TBMPCR Report ";
