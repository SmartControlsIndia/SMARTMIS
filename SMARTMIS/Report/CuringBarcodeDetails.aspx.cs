       ...
       // string moduleName = "PCRVIAdmin";
        ...
        protected void showReportDateMonthWise(string nfromDate, string ntoDate, string recipe, string tyredesign)
        {
            ...
            try
            {
                ...
                if(tyredesign=="PCR")
                {
                    myConnection.comm.CommandText = "select dbo.vCuringpcr.gtbarCode AS Barcode, dbo.vTbmPCR.recipeCode As RecipeName,convert(char(10), vCuringpcr.cycleUpdate, 105) AS CycleDate, convert(char(8), vCuringpcr.cycleUpdate, 108) AS CycleTime, dbo.vCuringpcr.wcName, dbo.vCuringpcr.shift,vCuringpcr.pressbarCode as Cavity,convert(char(10), vCuringpcr.dtandtime, 105)  as Scan_Date, convert(varchar(8), vCuringpcr.dtandtime, 108) as Scan_Time FROM  dbo.vCuringpcr INNER JOIN dbo.vTbmPCR ON dbo.vCuringpcr.gtbarCode = dbo.vTbmPCR.gtbarCode  where cycleUpdate>='" + nfromDate + "' and cycleUpdate<'" + ntoDate + "' order by cycleUpdate desc"; 
                }
                else
                {
                    myConnection.comm.CommandText = "select dbo.vCuringtbr.gtbarCode AS Barcode , dbo.vTbmtbR.recipeCode As RecipeName, convert(char(10), vCuringtbr.cycleUpdate, 105) AS CycleDate ,convert(char(10), vCuringtbr.cycleUpdate, 108) AS CycleTime , dbo.vCuringtbr.wcName, dbo.vCuringtbr.shift,convert(char(10), vCuringtbr.dtandtime, 105)  as Scan_Date, convert(varchar(8), vCuringtbr.dtandtime, 108) as Scan_Time FROM  dbo.vCuringtbr INNER JOIN dbo.vTbmtbR ON dbo.vCuringtbr.gtbarCode = dbo.vTbmtbR.gtbarCode  where dbo.vCuringtbr.dtandtime>='" + nfromDate + "' and dbo.vCuringtbr.dtandtime<'" + ntoDate + "' order by dbo.vCuringtbr.dtandtime desc"; 
                }
