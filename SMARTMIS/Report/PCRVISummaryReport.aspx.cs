...
{
    public partial class PCRVISummaryReport : System.Web.UI.Page
    {
        #region classes
        ...
        public int totalcheckedcount = 0, okcount = 0,NotOkCount=0, reworkcount = 0, ncmrcount = 0, totalrework = 0, treadfault = 0, sidewallfault = 0, beadfault = 0, carcassfault = 0, othersfault = 0;
        int status;
        ...
        private void loadData()
        {
            DataTable dt_vi = new DataTable();
                        ...
                        myConnection.comm.CommandText = "select wcID, curingRecipeID, status, defectStatusID As defectID from VInspectionPCR where ((dtandTime>=" + rToDate + " AND wcID in (select iD from wcmaster where vistage=1 and processID=9)";
                        ...
                        myConnection.comm.CommandText = "select Distinct iD, name from wcMaster where VIstage=1 and processID=9";
                        myConnection.reader = myConnection.comm.ExecuteReader();
        ...
        protected void ViewButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowWarning.Visible = true;
                ShowWarning.Text = "<table style=\"padding:4px;\"><tr><td width=20%><img src='../images/exclamation.png' height=\"30\" /></td><td width=80%><strong> <font color=#9F6000>You cannot select data of more than 2 days!!!</font></strong></td></tr></table>";
            }
            else
                showVIReport();   
        }
        ...
        private void fillBarCodeDetailGridView(string recipecode,string status)
        {
            try
            {
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "select wcname, description, gtbarCode AS vi_gtbarCode from vVisualInspectionPCR where status='" + status + "' AND   wcID in (select iD from wcmaster where vistage=1 and processID=9) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "'" + recipecode;
            ...
            try
            {
                ...
                else
                {
                   // rFromDate = formatfromDate(rFromDate.Replace(" 07:00:00", ""));
                    ...
                    from vvisualInspectionPCR where  wcID in (select iD from wcmaster where vistage=1 and processID=9) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "' order by viTime desc";
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                    dt.Load(myConnection.reader);
                    myConnection.conn.Close();
                    myConnection.comm.Dispose();
