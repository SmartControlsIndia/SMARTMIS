...
{
    public partial class TBRVISummaryReport2 : System.Web.UI.Page
    {
        #region classes
        ...
        private void loadData()
        {
            DataTable dt_vi = new DataTable();
                        ...
                        myConnection.comm.CommandText = "select wcID, curingRecipeID, status, defectID from TBRVisualInspection where ((dtandTime>=" + rToDate + " AND wcID IN (select  ID from wcmaster where vistage=1 and processID=6)";
                        ...
                        myConnection.comm.CommandText = "select Distinct iD, name from wcMaster where ID IN (select  ID from wcmaster where vistage=1 and processID=6)";
                        myConnection.reader = myConnection.comm.ExecuteReader();
    ...
    .Where(row => row.Cells[0].Text == "Total");
                ...
                switch(getdisplaytype)
                {
                    ...
                    case "Percent":
                        foreach (var item in items)
                        {
                            ...
                            dr[3] = (data.Count(d => d.status == 2) * 100 / Convert.ToInt32(dr[1])) + "%";
                            dr[4] = (data.Count(d => d.status == 3) * 100 / Convert.ToInt32(dr[1])) + "%";
        ...
        private void fillBarCodeDetailGridView(string recipecode,string status)
        {
            try
            {
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "select wcname, description, gtbarCode AS vi_gtbarCode from vTBRVisualInspectionReport where status='" + status + "' AND wcID IN (select distinct wcID from tbrvisualinspection where status=1) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "'" + recipecode;
            ...
            try
            {
                ...
                else
                {
                   // rFromDate = formatfromDate(rFromDate.Replace(" 07:00:00", ""));
                    ...
                    string[] head_array = new string[] { "S. No.", "visualWCName", "TyreSize", "Barcode", "Status", "DefectAreaName", "Defectname", "Remark", "shift","InspectorName","VIDate","VITime", "PressNo", "MouldNo", "Cure_Date", "Cure_Time", "tbmWCName", "TBM_Date", "TBM_Time", "BuilderName" };
                    ...
                    from vTBRVisualInspectionReport where WCID IN (select ID from wcmaster where vistage=1 and processID=6) AND dtandTime>='" + from_date + "' AND dtandTime<'" + to_date + "' AND status<>'1'";
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                    dt.Load(myConnection.reader);
                    myConnection.conn.Close();
                    myConnection.comm.Dispose();
                    ...
                    myConnection.open(ConnectionOption.SQL);
                    myConnection.comm = myConnection.conn.CreateCommand();
                    myConnection.comm.CommandText = "SELECT gtbarCode AS tbm_gtbarcode, wcName AS tbmWCName, CAST(dtandTime AS DATE) AS TBM_Date, convert(char(8), dtandTime, 108) AS TBM_Time, firstName + ' ' + lastName As BuilderName FROM vTbmTBR WHERE dtandTime>='" + tempfromdt + "' AND dtandTime<'" + to_date + "'";
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                    tbmdt.Load(myConnection.reader);
                    myConnection.conn.Close();
                    myConnection.comm.Dispose();
