                ...
                else
                {
                  ...
                  //  convert(char(8), dtandTime, 108) >= '15:00:00.000' AND convert(char(8), dtandTime, 108) <= '22:59:59.999' 
                   ...
                   // myWebService.writeLogs(myConnection.comm.CommandText, System.Reflection.MethodBase.GetCurrentMethod().Name, Path.GetFileName(Request.Url.AbsolutePath));
                    ...
                    myConnection.comm.CommandText = " Select tbm_gtbarcode,tbmWCName,TBM_Date,TBM_Time,BuilderName,BuilderName2,BuilderName3,SpecWeight from ( SELECT gtbarCode AS tbm_gtbarcode, wcName AS tbmWCName, convert(char(10), dtandTime, 105) AS TBM_Date, convert(char(8), dtandTime, 108) AS TBM_Time, firstName + ' ' + lastName As BuilderName, isnull( (select firstName from manningMaster where iD= manningID2),'unknown') as BuilderName2, isnull( (select firstName from manningMaster where iD= manningID3),'unknown') as BuilderName3,SpecWeight,row_number() over (partition by gtbarCode order by dtandTime desc) as rono FROM vTbmpcr WHERE dtandTime>='" + tempfromdt + "' AND dtandTime<'" + to_date + "')  as t where rono = 1 ";
                    myConnection.comm.CommandTimeout = 160;
                    myConnection.reader = myConnection.comm.ExecuteReader(CommandBehavior.CloseConnection);
                   // myWebService.writeLogs(myConnection.comm.CommandText, System.Reflection.MethodBase.GetCurrentMethod().Name, Path.GetFileName(Request.Url.AbsolutePath));
