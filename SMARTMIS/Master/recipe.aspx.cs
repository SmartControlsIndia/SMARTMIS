        ...
        private void Page_Init(object sender, System.EventArgs e) 
        {
             ...
             protected void DropDownList_SelectedIndexChanged(object sender, EventArgs e)
            {
                try
                {
                    ...
                    else if (((DropDownList)sender).ID == "DesignDropDownList1")
                    {
                        Label1.Text = DesignDropDownList1.SelectedItem.ToString();
                    }
                }
                catch(Exception exp)
                {
        ...
        #region User Defined Function
            ...
            private void fillGridviewProcessWise()
            {
                try
                {
                    ...
                    myConnection.open(ConnectionOption.SQL);
                    myConnection.comm = myConnection.conn.CreateCommand();
                    myConnection.comm.CommandText = "select RecipeName,ProcessName,ProductTypeName, OemName,RecipeNo,SpecWeight,TBRPCode, TyreSize,TyreDesign,Description ,SAPMaterialCode,upperWeight,lowerWeight,TBRTUOCode,TUOEnable  ,id as rid  ,id,updatedUserID,convert(char(10), lastUpdatedDate, 105) AS LastUpdatedDATE,SUBSTRING(lastUpdatedDate, 11, CHARINDEX(' ', lastUpdatedDate)) AS LastUpdatedTIME from  vRecipeMaster where processName='" + processDropDownList.SelectedValue.ToString() + "'order by id desc";
                    myConnection.reader = myConnection.comm.ExecuteReader();
                    ...
                    recipeGridView.Rows[0].Cells[17].Width = new Unit(5, UnitType.Percentage);
                    recipeGridView.Rows[0].Cells[18].Width = new Unit(5, UnitType.Percentage);
                    recipeGridView.Rows[0].Cells[19].Width = new Unit(5, UnitType.Percentage);
            ...
            private void fillGridView()
            {
                ...
                myConnection.open(ConnectionOption.SQL);
                myConnection.comm = myConnection.conn.CreateCommand();
                myConnection.comm.CommandText = "select RecipeName,ProcessName,ProductTypeName, OemName,RecipeNo,SpecWeight,TBRPCode, TyreSize,TyreDesign,Description ,SAPMaterialCode,upperWeight,lowerWeight,TBRTUOCode,TUOEnable  ,id as rid  ,id,updatedUserID ,convert(char(10), lastUpdatedDate, 105) AS LastUpdatedDATE,SUBSTRING(lastUpdatedDate, 11, CHARINDEX(' ', lastUpdatedDate)) AS LastUpdatedTIME from vRecipeMaster order by id desc";
                myConnection.reader = myConnection.comm.ExecuteReader();
                ...
                if (recipeIDLabel.Text.Trim() == "0")
                {
                    if ((validation() <= 0) && (myWebService.IsRecordExist("recipeMaster", "name", "WHERE name = '" + recipeNameTextBox.Text.Trim() + "'", out notifyIcon) == false))
                    {
                        try
                        {
                            ...
                            myConnection.comm.CommandText = "Insert into recipeMaster (processID,productTypeID,oemID, name,recipeNo,specWeight,TBRPCode, tyreSize,tyreDesign, description,SAPMaterialCode,upperWeight,lowerWeight,TBRTUOCode,TUOEnable,updatedUserID,lastUpdatedDate) values (@processID,@productTypeID,@oemID, @name,@recipeNo,@specWeight,@tbrPcode, @recipeSize, @design,@description,@SAPMaterialCode,@upperWeight,@lowerWeight,@TBRTUOCode,@TUOEnable,@userId,@updateDateTime)";
                            ...
                            myConnection.comm.Parameters.AddWithValue("@userId", Session["userID"].ToString());
                            myConnection.comm.Parameters.AddWithValue("@updateDateTime", DateTime.Now.ToString());
                   ...
                   // {
                    if (validation() <= 0)
                    {
                        try
                        {
                            myConnection.open(ConnectionOption.SQL);
                            myConnection.comm = myConnection.conn.CreateCommand();
                            myConnection.comm.CommandText = "Update recipeMaster SET productTypeID=@productTypeID,oemID=@oemID,recipeNO=@recipeNo,specWeight=@specWeight,TBRPcode=@tbrPcode, tyreSize=@recipeSize, tyreDesign = @design,description = @description,SAPMaterialCode=@SAPMaterialCode,upperWeight=@upperWeight,lowerWeight=@lowerWeight,TBRTUOCode=@TBRTUOCode,TUOEnable = @TUOEnable,updatedUserID=@userId,lastUpdatedDate=@updateDateTime WHERE (iD = @iD)";
                           // myConnection.comm.Parameters.AddWithValue("@processID", Convert.ToInt32(recipeProcessIDLabel.Text.Trim()));
                            myConnection.comm.Parameters.AddWithValue("@productTypeID", Convert.ToInt32(recipeProductTypeIDLabel.Text.Trim()));
                            myConnection.comm.Parameters.AddWithValue("@oemID", Convert.ToInt32(oemIDLabel.Text.Trim()));
                            //myConnection.comm.Parameters.AddWithValue("@name", recipeNameTextBox.Text.Trim());
